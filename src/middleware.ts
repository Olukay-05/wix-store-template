import { createClient, OAuthStrategy, Tokens } from "@wix/sdk";
import { NextRequest, NextResponse } from "next/server";
import { env } from "./env";
import { WIX_SESSION_COOKIE } from "./lib/constants";

const wixClient = createClient({
  // @ts-ignore - Temporary until SDK types are updated
  siteId: env.NEXT_PUBLIC_WIX_SITE_ID,
  auth: OAuthStrategy({
    clientId: env.NEXT_PUBLIC_WIX_CLIENT_ID,
  }),
});

export async function middleware(request: NextRequest) {
  const cookies = request.cookies;
  const sessionCookie = cookies.get(WIX_SESSION_COOKIE);

  let sessionTokens = sessionCookie
    ? (JSON.parse(sessionCookie.value) as Tokens)
    : await wixClient.auth.generateVisitorTokens();

  if (sessionTokens.accessToken.expiresAt < Math.floor(Date.now() / 1000)) {
    try {
      sessionTokens = await wixClient.auth.renewToken(
        sessionTokens.refreshToken,
      );
    } catch (error) {
      sessionTokens = await wixClient.auth.generateVisitorTokens();
    }
  }

  request.cookies.set(WIX_SESSION_COOKIE, JSON.stringify(sessionTokens));

  const res = NextResponse.next({ request });

  res.cookies.set(WIX_SESSION_COOKIE, JSON.stringify(sessionTokens), {
    maxAge: 60 * 60 * 24 * 14,
    secure: process.env.NODE_ENV === "production",
  });

  return res;
}

export const config = {
  matcher: ["/((?!_next/static|_next/image|favicon.ico).*)"],
};

// import { createClient, OAuthStrategy, Tokens } from "@wix/sdk";
// import { NextRequest, NextResponse } from "next/server";
// import { env } from "./env";
// import { WIX_SESSION_COOKIE, WIX_SITE_ID_COOKIE } from "./lib/constants";

// const wixClient = createClient({
//   auth: OAuthStrategy({ clientId: env.NEXT_PUBLIC_WIX_CLIENT_ID }),
// });

// export async function middleware(request: NextRequest) {
//   const cookies = request.cookies;
//   const sessionCookie = cookies.get(WIX_SESSION_COOKIE);
//   const siteIdCookie = cookies.get(WIX_SITE_ID_COOKIE);

//   let sessionTokens = sessionCookie
//     ? (JSON.parse(sessionCookie.value) as Tokens)
//     : await wixClient.auth.generateVisitorTokens();

//   if (sessionTokens.accessToken.expiresAt < Math.floor(Date.now() / 1000)) {
//     try {
//       sessionTokens = await wixClient.auth.renewToken(
//         sessionTokens.refreshToken,
//       );
//     } catch (error) {
//       sessionTokens = await wixClient.auth.generateVisitorTokens();
//     }
//   }

//   let siteId = siteIdCookie ? siteIdCookie.value : null;
//   if (!siteId) {
//     try {
//       const response = await fetch(
//         "https://www.wixapis.com/site-properties/v1/properties",
//         {
//           method: "GET",
//           headers: {
//             Authorization: `Bearer ${sessionTokens.accessToken.token}`,
//             "Content-Type": "application/json",
//           },
//         },
//       );
//       const data = await response.json();
//       siteId = data.siteId;
//     } catch (error) {
//       console.error("Error fetching siteId:", error);
//     }
//   }

//   request.cookies.set(WIX_SESSION_COOKIE, JSON.stringify(sessionTokens));
//   request.cookies.set(WIX_SITE_ID_COOKIE, siteId);

//   const res = NextResponse.next({ request });

//   res.cookies.set(WIX_SESSION_COOKIE, JSON.stringify(sessionTokens), {
//     maxAge: 60 * 60 * 24 * 14,
//     secure: process.env.NODE_ENV === "production",
//   });

//   res.cookies.set(WIX_SITE_ID_COOKIE, siteId, {
//     maxAge: 60 * 60 * 24 * 14,
//     secure: process.env.NODE_ENV === "production",
//   });

//   return res;
// }

// export const config = {
//   matcher: ["/((?!_next/static|_next/image|favicon.ico).*)"],
// };
